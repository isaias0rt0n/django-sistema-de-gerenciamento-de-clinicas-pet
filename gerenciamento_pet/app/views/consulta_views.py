...
def inserir_consulta(request, id):
    if request.method == "POST":
        ...
        if form_consulta.is_valid():
            ...
            consulta_nova = consulta.ConsultaPet(pet=pet, motivo_consulta=motivo_consulta, peso_atual=peso_atual, medicamento_atual=medicamento_atual,
                                                 medicamentos_prescritos=medicamentos_prescritos, exames_prescritos=exames_prescritos)
...
@login_required()
def listar_consulta_id(requst, id):
    consulta = consulta_service.listar_consulta(id)
    return render(requst, 'consultas/lista_consulta.html', {'consulta': consulta})
